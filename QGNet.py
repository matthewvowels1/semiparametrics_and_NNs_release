...
class Trainer(object):
        ...
        for it in range(self.iterations):
            ...
            if (it > 0) and ((it % self.test_iter == 0) or (it == (self.iterations - 1))):
                ...
                if self.net_type == 'Q':
                    ...
                    treg_loss = self.beta * self.treg(x_train[:len(x_val)], x_pred_train[:len(x_val)],
                                                      y_train[:len(x_val)], pred)
    ...
    def treg(self, x, pred_x, y, pred_y):
        ...
        if self.outcome_type == 'categorical':
            ...
            t_reg = self.bce_loss(y_pert, y).mean()
        #             torch.sum(- y * torch.log(y_pert) - (1 - y) * torch.log(1 - y_pert))
