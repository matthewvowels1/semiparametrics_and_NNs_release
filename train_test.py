...
class TrainTest(object):
	...
	def train_test(self):
		...
		upd_submod = np.nan
		upd_psi_treg = np.nan
		upd_treg_submod = np.nan
		biased_psi_halfSL = np.nan
		upd_1s_halfSL = np.nan
		upd_submod_halfSL = np.nan
		...
		while (i - self.starting_iter) < self.num_runs:
			...
			try:
				...
				if self.run_NN or self.run_treg or self.run_NN_SL:
					...
					if self.multinet:
						gtuner = MultiNetTuner(x=x, y=y, z=z, net_type='G', test_iter=self.test_iter,
						                       ...
						                       data_masking=self.data_masking,
						                       test_loss_plot=test_loss_plot, layerwise_optim=self.layerwise_optim,
						                       device=self.device)
					else:
						gtuner = Tuner(x=x, y=y, z=z, net_type='G', test_iter=self.test_iter, calibration=0,
						               device=self.device,
						               trials=self.num_tuning_trials, use_beta=False, use_t=use_t,
					...
					print('Tuning Q')
					if self.multinet:
						qtuner = MultiNetTuner(x=x, y=y, z=z, x_pred=x_pred, net_type='Q', test_iter=self.test_iter,
						                       data_masking=self.data_masking, device=self.device,
						                       trials=self.num_tuning_trials, use_beta=False, use_t=use_t,
						                       test_loss_plot=test_loss_plot, layerwise_optim=self.layerwise_optim,
						                       output_type=self.output_type)
					else:
						qtuner = Tuner(x=x, y=y, z=z, x_pred=x_pred, net_type='Q', test_iter=self.test_iter,
					...
					print('Tuning Q + treg')
					if self.multinet:
						qtregtuner = MultiNetTuner(x=x, y=y, z=z, x_pred=x_pred, net_type='Q', test_iter=self.test_iter,
						                           data_masking=self.data_masking, device=self.device,
						                           trials=self.num_tuning_trials, use_beta=True, use_t=use_t,
						                           test_loss_plot=test_loss_plot, layerwise_optim=self.layerwise_optim,
						                           output_type=self.output_type, )
					else:
						qtregtuner = Tuner(x=x, y=y, z=z, x_pred=x_pred, net_type='Q', test_iter=self.test_iter,
					...
					qtregiters = qtregbest_params[
						'iters']  # override the early stopping iter (will still use early stopping)
				...
				if self.run_NN or self.run_treg or self.run_NN_SL:
					if self.multinet:
						qnet = QMultiNet(input_size=input_size_Q, num_layers=qlayers, device=self.device,
						                 ...
						                 output_type=output_type_Q, dropout=qdropout, use_t=use_t,
						...
						gtrainer = MultiNetTrainer(net=gnet, net_type='G', beta=0.0, iterations=giters,
						                           ...
						                           weight_reg=gweight_reg, test_loss_plot=test_loss_plot, split=False,
						                           layerwise_optim=self.layerwise_optim)
					...
					if self.multinet:
						qtrainer = MultiNetTrainer(net=qnet, net_type='Q', beta=0.0, iterations=qiters,
						                           ...
						                           test_loss_plot=test_loss_plot, split=False,
						                           layerwise_optim=self.layerwise_optim)
						...
						qtrainer = Trainer(net=qnet, net_type='Q', beta=0.0, iterations=qiters,
						                   outcome_type=output_type_Q,
						                   batch_size=qbatch_size, test_iter=1000, lr=qlr, weight_reg=qweight_reg,
						                   device=self.device,
						                   test_loss_plot=test_loss_plot, calibration=self.calibration, split=False)
				...
				if self.run_treg:
					...
					if self.multinet:
						qnet = QMultiNet(input_size=input_size_Q, num_layers=qtreglayers, device=self.device,
						                 ...
						                 output_type=output_type_Q, dropout=qtregdropout, use_t=use_t,
						                 layerwise_optim=self.layerwise_optim).to(self.device)
						...
						# redefine Q trainer (treg enabled)
						qtrainer = Trainer(net=qnet, net_type='Q', beta=1.0, iterations=qtregiters,
						                   outcome_type=output_type_Q, device=self.device,
						                   batch_size=qtregbatch_size, test_iter=1000, lr=0.0001,
						                   weight_reg=qtregweight_reg,
						                   test_loss_plot=test_loss_plot, calibration=self.calibration, split=False)
				...
				if self.run_treg_SL:
					...
					#  submodel approach
					Q1_star_solve_treg_submod, Q0_star_solve_treg_submod = submodel(x_, y_, Q1, Q0, Q10, G10,
					                                                                outcome_type=self.output_type)
					upd_treg_submod = (Q1_star_solve_treg_submod - Q0_star_solve_treg_submod).mean()
				...
				if self.run_NN_SL:
					...
					if self.multinet:
						qtrainer = MultiNetTrainer(net=qnet, net_type='Q', beta=0.0, iterations=qiters,
						                           outcome_type=output_type_Q,
						                           batch_size=qbatch_size, test_iter=1000, lr=qlr,
						                           weight_reg=qweight_reg, device=self.device,
						                           test_loss_plot=test_loss_plot, split=False,
						                           data_masking=self.data_masking, layerwise_optim=self.layerwise_optim)
					...
					biased_psi_halfSL = (Q1 - Q0).mean()

					# one step approach
					Q1_star_1s_halfSL, Q0_star_1s_halfSL = one_step(x_, y_, Q0, Q1, G10)
					upd_1s_halfSL = (Q1_star_1s_halfSL - Q0_star_1s_halfSL).mean()

					#  submodel approach
					Q1_star_solve_submod_halfSL, Q0_star_solve_submod_halfSL = submodel(x_, y_, Q1, Q0, Q10, G10,
					                                                                    outcome_type=self.output_type)
					upd_submod_halfSL = (Q1_star_solve_submod_halfSL - Q0_star_solve_submod_halfSL).mean()
				...
				if self.run_treg_SL:
					...
					upd_psi_treg_halfSL = (Q1 - Q0).mean()

					# ------------------------PREVIOUS TREG MODEL WITH SUBMODEL UPDATE COMBINED and SL ------------------------------------------------------------
					print('Combining TREG with submodel update')
					Q1_star_solve_treg_submod_halfSL, Q0_star_solve_treg_submod_halfSL = submodel(x_, y_, Q1, Q0, Q10,
					                                                                              G10,
					                                                                              outcome_type=self.output_type)
					upd_treg_submod_halfSL = (
								Q1_star_solve_treg_submod_halfSL - Q0_star_solve_treg_submod_halfSL).mean()
				...
				if self.run_LR:
					...
					if self.output_type == 'categorical':
						...
						Q1 = np.clip(Q.predict_proba((np.concatenate([x_int1_, z_], 1)))[:, 1:], a_min=0.025,
						             a_max=0.975)
						Q0 = np.clip(Q.predict_proba((np.concatenate([x_int0_, z_], 1)))[:, 1:], a_min=0.025,
						             a_max=0.975)
					...
					biased_psi_LR = (Q1 - Q0).mean()

					G = LogisticRegression().fit(z_, x_[:, 0])
					G10 = np.clip(G.predict_proba(z_), a_min=0.025, a_max=0.975)[:, 1:]

					# submodel approach
					Q1_star_solve_submod_LR, Q0_star_solve_submod_LR = submodel(x_, y_, Q1, Q0, Q10, G10,
					                                                            outcome_type=self.output_type)
					upd_submod_LR = (Q1_star_solve_submod_LR - Q0_star_solve_submod_LR).mean()
				...
				if self.run_SL:
					...
					# submodel approach
					Q1_star_solve_upd_submod_SL, Q0_star_solve_upd_submod_SL = submodel(x_, y_, Q1, Q0, Q10, G10,
					                                                                    outcome_type=self.output_type)

					# one step approach
					Q1_star_upd_1s_SL, Q0_star_upd_1s_SL = one_step(x_, y_, Q0, Q1, G10)
					upd_submod_SL = (Q1_star_solve_upd_submod_SL - Q0_star_solve_upd_submod_SL).mean()
					upd_1s_SL = (Q1_star_upd_1s_SL - Q0_star_upd_1s_SL).mean()

				sample_truth.append(sample_psi)
				estimates_naive.append(biased_psi_nn)
				estimates_upd_1s.append(upd_1s)
				estimates_upd_submod.append(upd_submod)

				estimates_upd_treg.append(upd_psi_treg)
				estimates_upd_treg_submod.append(upd_treg_submod)
				estimates_naive_halfSL.append(biased_psi_halfSL)
				estimates_upd_1s_halfSL.append(upd_1s_halfSL)

				estimates_upd_submod_halfSL.append(upd_submod_halfSL)
				estimates_upd_treg_halfSL.append(upd_psi_treg_halfSL)
				estimates_upd_treg_submod_halfSL.append(upd_treg_submod_halfSL)
			...
			if self.multinet:
				...
				df_betas_g.to_csv(
					self.fn + '{}_betas_G_t_predictor_{}_datarand_{}_{}_{}_datamask_{}_{}_{}_start_iter_{}.csv'.format(
						self.run, use_t, self.data_rand, self.dataset,
						self.multinet, self.data_masking, current_time, self.N, self.starting_iter), index=False)
				df_betas_q.to_csv(
					self.fn + '{}_betas_Q_t_predictor_{}_datarand_{}_{}_{}_datamask_{}_{}_{}_start_iter_{}.csv'.format(
						self.run, use_t, self.data_rand, self.dataset,
						self.multinet, self.data_masking, current_time, self.N, self.starting_iter), index=False)
