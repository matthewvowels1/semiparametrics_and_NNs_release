        ...
        if self.layerwise_optim:
            ...
            for l in range(num_layers):
                ...
                if net_type == 'Q':
                    ...
                    optim_ = optim.SGD([{"params":list(layer_params) + list(out_layer_pos_params) + list(out_layer_neg_params),
                                           "lr":lr}])
                elif net_type == 'G':
        ...
        for it in range(self.iterations):
            ...
            if self.net_type == 'Q':
                ...
                for nl in range(self.net.num_layers):
                    ...
                    treg_loss = (mask * self.beta * self.treg(x_batch, x_pred_batch, y_batch, pred[nl, :, :])).mean()
                    if self.outcome_type == 'categorical':
                        ll =  (mask * self.bce_loss(pred[nl, :, :], y_batch)).mean() + treg_loss
                    else:
                        ll = (mask * self.mse_loss(pred[nl, :, :], y_batch)).mean() + treg_loss
            ...
            if (it > 0) and ((it % self.test_iter == 0) or (it == (self.iterations - 1))):
                ...
                if self.net_type == 'Q':
                    ...
                    for nl in range(self.net.num_layers):
                        ...
                        treg_loss = (mask * self.beta * self.treg(x_train[:len(x_val)], x_pred_train[:len(x_val)],
                                                                      y_train[:len(x_val)], pred[nl, :, :])).mean()
                        if self.outcome_type == 'categorical':
                            ll = (mask * self.bce_loss(pred[nl, :, :], y_train[:len(x_val)])).mean() + treg_loss
                        else:
                            ll = (mask * self.mse_loss(pred[nl, :, :], y_train[:len(x_val)])).mean() + treg_loss
    ...
    def test(self, model, x, y, z, x_pred=None):
        ...
        if self.net_type == 'Q':
            ...
            for nl in range(model.num_layers):
                gts = y
                treg_loss += self.beta * self.treg(x, x_pred, gts, pred[nl, :, :]).mean()
                if self.outcome_type == 'categorical':
